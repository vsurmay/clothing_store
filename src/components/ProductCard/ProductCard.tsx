...
const ProductCard: React.FC<ProductCardProps> = ({ data }) => {
  ...
  const { url, loading } = useFireBaseStorage(
    "clother/" + data.images[activeColor]
  );
  ...
  return (
    <Link to={`/shop/product/${data.id}`}>
      <div className={classes.card}>
        {!loading ? (
          <Image preview={false} width={344} height={450} src={url} />
        ) : (
          <Image
            preview={false}
            width={344}
        ...
        <div className={classes.info}>
          ...
          {data.discount ? (
            <h3 className={`${classes.newPrice} ${classes.price}`}>
              {calculationNewPrice(data.price, data.discount).toFixed(2)}
              <span className={classes.oldPrice}>
                {calculationNewPrice(data.price, 0).toFixed(2)}
              </span>
            </h3>
          ) : (
